...
import { SponsorshipAdopted } from "../models/sponsorship_adopted.js";
...
export const createRequest = async (req, res) => {
    try {
        const { userId, animalId, questions, state_id } = req.body;
        const {
            reason,
            experience,
            time,
        ...
        if (!userId || !animalId) {
            return res
                .status(400)
                .json({ message: "userId y animalId son requeridos" });
        }
        ...
        if (!user || !animal) {
            return res
                .status(404)
                .json({ message: "Usuario o animal no encontrado" });
        }
        ...
        for (const field of fields) {
            if (typeof field.value !== "string" || field.value.trim() === "") {
                console.error(
                    `Error en el campo "${field.label}": El valor es inválido (${field.value})`
                );
                return res.status(400).json({
                    message: `El campo "${field.label}" tiene un valor inválido.`,
                });
            }
        }
        ...
        const questionAnswers = [
            {
                question: "1. ¿Por qué te gustaría apadrinar un animal?",
                answer: reason,
            },
            {
                question: "2. ¿Has apadrinado algún animal antes?",
                answer: experience,
            },
            {
                question:
                    "3. ¿Cuánto tiempo estás dispuesto a comprometerte para el apadrinamiento?",
                answer: time,
            },
            {
                question:
                    "4. ¿Estás dispuesto a cubrir gastos asociados con el cuidado animal?",
                answer: cost,
            },
            {
                question:
                    "5. ¿Te gustaría recibir actualizaciones periódicas sobre el animal que apadrines?",
                answer: notification,
            },
            {
                question:
                    "6. ¿Estás interesado en participar en actividades o eventos organizados por la fundación?",
                answer: events,
            },
            {
                question:
                    "7. ¿Cuál es tu disponibilidad para participar en el cuidado y apoyo del animal?",
                answer: availability,
            },
            {
                question:
                    "8. ¿Tienes alguna pregunta o inquietud sobre el proceso de apadrinamiento?",
                answer: question,
            },
        ];
        ...
        res
            .status(201)
            .json({ message: "Solicitud creada y correo enviado exitosamente" });
    } catch (error) {
...
export const getRequests = async (req, res) => {
    try {
        ...
        res
            .status(200)
            .json({ message: "Solicitudes obtenidas con éxito", data: requests });
    } catch (error) {
        console.error(error);
...
export const updateRequestStatus = async (req, res) => {
    try {
        ...
        if (!status || !["pendiente", "aprobada", "rechazada"].includes(status)) {
            return res
                .status(400)
                .json({ message: "Estado inválido o no proporcionado" });
        }
        ...
        if (status === "rechazada") {
            ...
            animal.state_id = request.state_id;
            await animal.save();

            const sponsorship_adopted = await SponsorshipAdopted.create({
                animal_id: request.animal_id,
                sponsorship_start_date: new Date(),
                user_id: request.user_id,
                status_id: request.state_id,
            });

            console.log(sponsorship_adopted);
        ...
        res
            .status(200)
            .json({ message: "Estado actualizado con éxito", data: request });
    } catch (error) {
        console.error(error);
